...
public class Database {
    ...
    private boolean checkMaterials(String productName, int amount) {
		...
		try {
			...
			if(recipeMaterials.isEmpty()) {
				System.out.println("recipeMaterials empty");
    ...
    private boolean updateMaterials(String productName, int amount) {
    	String sql = 
				"SELECT *\n"
			+	"FROM " + TABLE_RECIPES + "\n"
			+	"WHERE " + KEYWORD_RECIPE_NAME + " = '" + productName + "';";
		...
		for(Recipe r : recipeMaterials) {
			sql =
					"UPDATE " + TABLE_MATERIALS + "\n"
				+	"SET " + KEYWORD_AMOUNT + " = " + KEYWORD_AMOUNT + " - " + r.getAmount() * amount + ", " + KEYWORD_TIMESTAMP + " = CURRENT_TIMESTAMP\n"
				+	"WHERE " + KEYWORD_MATERIAL_NAME + " = '" + r.getMaterial_name() + "';";
			...
			try {
				...
				if(result == 0) {
					System.out.println("Could not update material");
    ...
    public boolean createPallets(String productName, int amount) {
    	...
    	try {
    		...
    		if(failed || !updateMaterials(productName, amount)) {
    			conn.rollback();
    		}
